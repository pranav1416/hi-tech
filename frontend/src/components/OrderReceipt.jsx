  ...
  useEffect(() => {
    async function saveOrderHistory() {
      ...
      if (response.data.products) {
        ...
        setOrderTotalMessage(
          'Order Total: ' + formatter.format(response.data.price.total)
        )

        setShippingAddr(
          response.data.shipping.address.addr1 +
            ' , ' +
            response.data.shipping.address.addr2
        )

        setShippingAddr2(
          response.data.shipping.address.city +
            ' , ' +
            response.data.shipping.address.state +
            ' . ' +
            response.data.shipping.address.zipcode
        )
                ...
                <div>
                  ...
                  <tbody>
                    <tr class='no_padding'>
                      ...
                      <td class='no_padding center_col'>
